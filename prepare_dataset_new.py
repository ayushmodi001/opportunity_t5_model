...
def get_mcql_dataset() -> List[Dict]:
    ...
    try:
        # Using MMLU computer science subset
        dataset = load_dataset("lighteval/mmlu", "college_computer_science")
        processed = []
        
        for split in ['validation', 'test']:  # MMLU has good validation/test sets
            for item in dataset[split]:
